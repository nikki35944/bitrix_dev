...
class Manager {
	...
	public function __construct($config = []){
		...
		$this->config = array_merge([
			...
			'ROOT_ID' => 72082,
			'DIRECTORIES' => [
				['ID' => false, 'NAME' => false, 'PATH' => false],
			],
			'OUT_DIRECTORY' => $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR . 'upload/xml/',
		], $config);
	}
					...
					if ($currentDate == $currentFileDate){
						...
						if(!$lastTimeFile || intval($lastTimeFile) < $currentFileTime){
							...
							try {
								...
								$refactorXml = $this->refactorEmployees($refactorXml);
	...
	public function refactorEmployees($xml){
